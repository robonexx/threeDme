import {
  ...
  export const ExperienceOffice = () => {
    ...
    useFrame((_, delta) => {
      textMaterial.current.opacity = lerp(
        ...
        currentPage === "home" || currentPage === "about" ? 1 : 0,
        delta * 1.5
      );
    });
    ...
    return (
      <>
        ...
        <Text
          font={"fonts/BasementGrotesque-Black.otf"}
          position-x={-1.3}
          position-y={-0.4}
          position-z={1}
          lineHeight={1}
        ...
        <group rotation-y={degToRad(-25)} position-x={3} position-y={-0.35}>
          <Office scale={0.6} html />
          <mesh ref={meshFitCameraStore} visible={false}>
            <boxGeometry args={[2, 1, 2]} />
            <meshBasicMaterial color="red" transparent opacity={0.5} />
          </mesh>
        </group>
        <mesh position-y={-0.38} rotation-x={-Math.PI / 2}>
  ...
  useFont.preload("fonts/BasementGrotesque-Black.otf");